...
<body class="font-roboto">
        ...
        <div class="h-screen overflow-hidden w-full">
            ...
            <div class="bg-[#262B3A] flex justify-between items-center py-3 px-10">
                <div class="text-white">
                    <p class="text-2xl font-semibold">Banned Customers</p>
                    <?php
                    $timestamp = time();
                ...
                <input type="text" name="" id="searchBannedCustomers" class="w-[800px] py-2 px-5 rounded outline-none" placeholder="Search...">
            </div>
            <!-- Search End-->
            ...
            <div class="px-[53px] py-8">
                ...
                <div class="flex items-center justify-end space-x-2 mb-2">
                    ...
                    <select name="" id="dropDownBannedCustomer" class="outline-none rounded py-1 px-3">
                        ...
                        <div class="h-[600px] overflow-y-scroll scrollbar-hide">
                            <table class="table text-textBlack border-separate space-y-6 text-sm w-[1200px] overflow-y-scroll">
                                ...
                                <tbody class="searchResult">
                                    <?php $num = 1; ?>
                                    <?php foreach ($bannedCustomersList as $bannedCustomer) { ?>
                                        <tr class="bg-[#fffafa]">
                                            <td class="p-3 text-center"><?= $num++; ?></td>
                                            <td class="p-3 text-center"><?= $bannedCustomer["c_name"] ?></td>
                                            <td class="p-3 text-center"><?= $bannedCustomer["c_email"] ?></td>
                                            <td class="p-3 text-center"><?= $bannedCustomer["c_phone"] ?></td>
                                            <td class="p-3 text-center"><?= $bannedCustomer["c_address"] ?></td>
                                            <td class="p-3 text-center"><?= $bannedCustomer["create_date"] ?></td>
                                            <td class="p-3 text-center ">
                                                <?php $cName = $bannedCustomer["c_name"] ?>
                                                <?php $cEmail = $bannedCustomer["c_email"] ?>
                                                <span class="banModal px-4 py-1 cursor-pointer bg-[#396C21] text-white rounded-md" onclick="permitCustomer('<?= $cName ?>','<?= $cEmail ?>')">PERMIT</span>
                                            </td>
                                        </tr>
                                    <?php } ?>
                                </tbody>
            ...
            <div class="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                ...
                <div class="relative w-[400px]  mx-auto mt-[120px]  bg-primary rounded-lg text-left overflow-hidden shadow-xl transform transition-all ">
                    <div class="bg-white px-4 pt-5 pb-4">
                        ...
                        <form action="../../Controller/customersList/permitCustomerController.php" method="post">
                            <div class="mt-3 ml-4 text-left">
                                <span class="text-center block pb-4 mt-7">You are about to permit the following user:</span>
                                <h1 class=" font-semibold mt-5">Name: <input id="permitName" name="c_Name" class="w-[200px] ml-[9px] font-normal outline-none" readonly></input></h1>
                                <h1 class="font-semibold mt-5"> Email: <input id="permitMail" name="c_Email" class="w-[240px] font-normal ml-[13px] outline-none" readonly></input></h1>
    ...
    <!-- End Modal Box -->
    <script>
        function permitCustomer(name, email) {
